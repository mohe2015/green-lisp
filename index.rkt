...
(let ((writer (new bit-writer%)))
  ...
  (send writer write-byte ET_EXEC) ;; e_type
  (send writer write-byte EM_X86_64) ;; e_machine
  (send writer write-byte EV_CURRENT) ;; e_version
  (send writer write-byte 1) ;; entrypoint) ;; e_entry
