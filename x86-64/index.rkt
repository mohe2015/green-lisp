    ...
    [(global-symbol symbol)
     #`(list (lambda (_) 0)
             ...
             (lambda (current-address)
               (list (new elf-symbol% [name #,(string->bytes/utf-8 (symbol->string (syntax-e #'symbol)))] [type 'func] [binding 'local] [section #".text"] [value current-address] [size 10])))
             )]))
  ...
  (syntax-case stx ()
    [(global-symbol symbol)
     #`(list (lambda (_) 0)
             ...
             (list)
             (lambda (current-address)
               (list (new elf-symbol% [name #,(string->bytes/utf-8 (symbol->string (syntax-e #'symbol)))] [type 'func] [binding 'global] [section #".text"] [value current-address] [size 10])))
             )]))
